...
void GLCD_SetPixels(const uint8_t X1, uint8_t Y1, const uint8_t X2, const uint8_t Y2, enum Color_t Color)
{
	...
	(Y1 < __GLCD_Screen_Height) && (Y2 < __GLCD_Screen_Height))
	{
		...
		for (i = 0 ; i < width ; i++)
		{
			...
			data = GLCD_BufferRead(__GLCD.X, __GLCD.Y);
			//Mask
			data = ((Color == w) ? (data | mask) : (data & ~mask));
			//Write
			GLCD_BufferWrite(__GLCD.X++, __GLCD.Y, data);
		}
		...
		{
			mask = ~(0xFF << (height - h));
			g_xy(X1, Y1 + __GLCD_Screen_Line_Height);
			for (i = 0 ; i < width ; i++)
			{
				//Read
				data = GLCD_BufferRead(__GLCD.X, __GLCD.Y);
...
void g_clear(void)
{
	g_fill(__GLCD.Mode == GLCD_Non_Inverted ? b : w);
}

void g_clearline(const uint8_t Line)
{
	if (Line < __GLCD_Screen_Lines)
	{
		uint8_t i, color;
		...
		g_xy(0, Line * __GLCD_Screen_Line_Height);
		for (i = 0 ; i < __GLCD_Screen_Width ; i++)
...
void g_line(uint8_t X1, uint8_t Y1, uint8_t X2, uint8_t Y2)
{
	enum Color_t Color=w;
	if ((X1 < __GLCD_Screen_Width) && (X2 < __GLCD_Screen_Width) &&
	(Y1 < __GLCD_Screen_Height) && (Y2 < __GLCD_Screen_Height))
	{
		if (X1 == X2)
...
void g_rec(const uint8_t X1, const uint8_t Y1, const uint8_t X2, const uint8_t Y2)
{
	enum Color_t Color=w;
	if ((X1 < __GLCD_Screen_Width) && (X2 < __GLCD_Screen_Width) &&
	(Y1 < __GLCD_Screen_Height) && (Y2 < __GLCD_Screen_Height))
...
void g_roundrec(const uint8_t X1, const uint8_t Y1, const uint8_t X2, const uint8_t Y2, const uint8_t Radius)
{
	enum Color_t Color=w;
	if ((X1<__GLCD_Screen_Width) && (X2<__GLCD_Screen_Width) &&
	(Y1<__GLCD_Screen_Height) && (Y2<__GLCD_Screen_Height))
...
void g_tri(const uint8_t X1, const uint8_t Y1, const uint8_t X2, const uint8_t Y2, const uint8_t X3, const uint8_t Y3)
{
...
void g_cir(const uint8_t CenterX, const uint8_t CenterY, const uint8_t Radius)
{
	enum Color_t Color=w;
...
void g_f_rec(const uint8_t X1, const uint8_t Y1, const uint8_t X2, const uint8_t Y2)
{
	enum Color_t Color=w;
	GLCD_SetPixels(X1, Y1, X2, Y2, Color);
}

void g_f_roundrec(const uint8_t X1, const uint8_t Y1, const uint8_t X2, const uint8_t Y2, const uint8_t Radius)
{
	...
	(Y1 < __GLCD_Screen_Height) && (Y2 < __GLCD_Screen_Height))
	{
		...
		//Fill center block
		g_f_rec(X1+Radius, Y1, X2 - Radius, Y2);
...
void g_f_tri(uint8_t X1, uint8_t Y1, uint8_t X2, uint8_t y2, uint8_t X3, uint8_t Y3  )
{
...
void g_f_cir(const uint8_t CenterX, const uint8_t CenterY, const uint8_t Radius)
{
	enum Color_t Color=w;
	...
	((CenterY + Radius) < __GLCD_Screen_Height))
	{
		...
		g_line(CenterX, CenterY - Radius, CenterX, CenterY + Radius);
...
void g_i_rec(uint8_t X1, uint8_t Y1, uint8_t X2, uint8_t Y2)
{
...
void g_font(const uint8_t *Name, const uint8_t Width, const uint8_t Height)
{
	enum PrintMode_t Mode=GLCD_Merge;
...
void printi(const int32_t Value)
{
	if (Value == 0)
	{
		printc('0');
...
void printd(double Value, const uint32_t Tens)
{
	if (Value == 0)
	{
	...
	else if ((Value >= (-2147483647)) && (Value < 2147483648))
	{
		...
		//Print decimal part
		printi((Value - (uint32_t)(Value)) * Tens);
	}
}
